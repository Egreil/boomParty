...
class AdminController extends AbstractController
{
    ...
    ): Response
    {
        ...
        if($csvForm->isSubmitted() && $csvForm->isValid()){
            ...
            $lecteurFichierCSV->lireFichierCSV($em,$this->getParameter('csv_directory').'/'.$newFilename,$mailer);
            $this->addFlash('success', 'Le fichier csv a été lu avec succès.');
        }

        return $this->render('admin/inscriptionFormulaireCSV.html.twig', [
            'csvForm'=>$csvForm
        ]);
    ...
    ): Response
    {
        ...
        if ($participantForm->isSubmitted() /*&& $participantForm->isValid()*/){
            $inscriptionMailingService->creationCompteVierge($em,null,$mailer,$participant);
            $this->addFlash('success', 'Le fichier csv a été lu avec succès.');
